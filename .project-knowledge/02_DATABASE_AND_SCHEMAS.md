...
**Indexes**: text index on `productName/sku/barcode`; partial unique on `sku` (`{deletedAt: null}`); compound `{category, status}`, `{status, currentStock}`; single-field `{currentStock}` (dashboard low-stock sort has no status predicate, so it can't use the compound index).

**`pre('save')`**: if new & no sku — populates category+unit, builds SKU:
`PRD-{categoryCode}-{unitCode}-{productCode}-{uniqueCode}` (first 3 letters of category name, unit abbreviation, initials of productName up to 3, last 6 of ObjectId). Throws if category/unit missing at save time.
...
Immutable ledger. `product` (ref, required), `type` (enum: `purchase_in`, `stock_in`, `stock_out`, `adjustment_in`, `adjustment_out`, `adjustment` — no `purchase_cancel`; cancelling only applies to pending POs and never affects stock), `quantity` (signed), `quantityBefore`, `quantityAfter`, `referenceId`, `referenceType` (enum: `Purchase`, `StockAdjustment`, null), `note`, `unitCost` (nullable, set on `purchase_in` receipts so the ledger keeps the historical cost even after the product's live `costPrice` is later updated), `createdBy` (ref User, required). Indexes: `{product, createdAt:-1}`, `{type:1}`, `{referenceId, referenceType}`.

### Notification (`notification_model.js`)

`user` (ref, required), `type` (enum: `low_stock`, `out_of_stock`, `purchase_received`, `purchase_cancelled`, `system`), `title`, `message`, `referenceId`, `referenceType` (enum: `Product`, `Purchase`, null), `isRead` (default false), `readAt`. Index: `{user, isRead, createdAt:-1}`. No schema hooks.

### Idempotency (`idempotency_model.js`)

Claims client-supplied idempotency keys. `user` (ref, required), `key` (string, required, max 128), `purchase` (ref Purchase, nullable — back-filled after the PO is created). Unique index: `{user, key}` — the winner of the claim creates the resource; concurrent losers get E11000 and poll briefly for `purchase`.
...
## 3. Database Selection & Connection (`server.js`)
...
The server **fails fast**: `connectWithRetry()` (3 attempts, 5s apart, `serverSelectionTimeoutMS: 10000`) then `process.exit(1)` with troubleshooting hints if MongoDB can't be reached. The HTTP listener only starts after the DB connection succeeds.
