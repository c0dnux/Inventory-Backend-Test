...
- **Signin** (`signin`): requires `email` + `password`; checks active flag, verifies bcrypt password; logs audit `login`; returns JWT. If the account is Google-linked (no local password) → returns a friendly error telling the user to log in with Google.
- **Google Signin** (`googleSignin`, `POST /api/v1/auth/google`): frontend sends a Google ID token (`body.credential` or `body.idToken`); backend verifies signature + `aud` with `google-auth-library` using `GOOGLE_CLIENT_ID` (no secret needed). Requires `payload.email_verified`. **Find-or-create by verified email**: no user → creates one (Staff role, `active: true`, `authProviders: [{provider:'google', providerId: sub}]`); existing local user → links Google (`addProvider`); existing user with a *different* `providerId` → 401. Logs audit `login`, returns JWT. Email/password `signup` rejects emails already claimed by Google (409).
