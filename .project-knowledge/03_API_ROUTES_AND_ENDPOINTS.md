...
| ------ | ---------------- | ------------------ | ---------------------------------------------- |
| POST   | /signup          | signup             | public + `authLimiter`                         |
| POST   | /activate        | activateAccount    | public + `authLimiter`                         |
| POST   | /login           | signin             | public + `authLimiter`                         |
| POST   | /google          | googleSignin       | public + `authLimiter`                         |
| POST   | /refresh         | refreshAccessToken | public + `refreshLimiter` (rotation)           |
| POST   | /forget-password | forgetPassword     | public + `authLimiter`                         |
| POST   | /reset-password  | resetPassword      | public + `authLimiter`                         |
| POST   | /logout          | logout             | public (revokes refresh token, clears cookies) |
| POST   | /profile         | profile            | protect                                        |
| POST   | /update-password | updatePassword     | protect                                        |
...
| GET    | /dashboard | getInventoryDashboard | protect + products:read (cache 60s) |

Note: `/dashboard` is mounted **before** `/:id` so the string `"dashboard"` is never captured as an id.
...
Applies to all list endpoints: `?page=`, `?limit=` (default 50, max 100), `?sort=field,-field`, `?fields=a,b,c`, Mongo operators `gte|gt|lte|lt`, and **`?search=`** (MongoDB `$text` search — only meaningful on models with a text index, currently just Products). List responses include a `totalCount` field (full matching count, unaffected by pagination) on products, purchases, suppliers, adjustments, movements, and audits.
Example: `GET /api/v1/products?search=paracetamol&category=xyz&sort=-createdAt&page=2&limit=20`
